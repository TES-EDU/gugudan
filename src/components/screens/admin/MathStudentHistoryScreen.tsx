...
export default function StudentHistoryScreen({ studentName, onBack, onReportClick }: Props) {
  ...
  const frequentWrong = useMemo(() => {
    const map = new Map<string, { expression: string; correctAnswer: number | string; wrongCount: number; totalSeen: number }>();
    rows.forEach(r => {
      (r.incorrect_answers ?? []).forEach(w => {
        const k = w.expression;
        const e = map.get(k) ?? { expression: w.expression, correctAnswer: w.correctAnswer, wrongCount: 0, totalSeen: 0 };
    ...
    <div className="bg-sb-surface border border-sb-line rounded-2xl p-5">
      <div className="flex items-center gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[13px] text-sb-muted mt-0.5">TES MATH 응시자</div>
        </div>
        <button
  ...
  return (
    <div className="min-h-screen bg-sb-bg flex flex-col">
      <header className="bg-sb-surface border-b border-sb-line h-14 px-5 flex items-center gap-2.5 shrink-0">
        ...
        <span className="text-sm font-extrabold text-sb-ink -tracking-tight">TES MATH</span>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2.5 px-3 py-2.5 bg-sb-bg rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-sb-muted">{w.correctAnswer}</div>
                      </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-sb-muted mt-0.5">
                  {rows[0] ? `최근 응시 ${new Date(rows[0].created_at ?? '').toLocaleDateString('ko-KR')}` : 'TES MATH 응시자'}
                </div>
              </div>
            </div>
                  ...
                  {frequentWrong.map((w, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-sb-wrong-pale/40">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-sb-muted truncate">{w.correctAnswer}</div>
                      </div>
